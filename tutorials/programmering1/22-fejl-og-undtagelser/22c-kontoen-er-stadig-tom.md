...
1. Opret istedet en ny undtagelsesklasse `InsufficientFundsException`, der arver fra `Exception`, som gør at `InsufficientFundsException` bliver en checked exception.
...
8. Prøv at lade `InsufficientFundsException` arve fra `RuntimeException` i stedet for `Exception`. Hvad sker der nu? Hvad er forskellen på at arve fra `Exception` og `RuntimeException`?