...
1. Opret et nyt IntelliJ-projekt. Lav en klasse Person med følgende felter:
   ...
   - `Person father`
   - `Person mother`
2. Lav en konstruktør, der initialiserer `name` og `gender` og lader `father` og `mother` være `null` for nu.
3. Tilføj nu getter og setter metoder for alle felterne, så du kan tilføje forældre til en `Person` efter at den er oprettet.
4. Tilføj en metode `toString()`, der returnerer fx "Beate ♀" eller "Allan ♂".
5. Prøv at lave et simpelt stamtræ for dig selv og din familie (eller det danske kongehus) ved at oprette `Person`-objekter og forbinde dem via `mother`- og `father`-feltet, vha. setter-metoderne, eksempelvis:

```java
Person beate = new Person("Beate", Gender.FEMALE);
Person allan = new Person("Allan", Gender.MALE);

beate.setFather(allan);
```
